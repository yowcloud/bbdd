...
-- Trigger para actualizaciones en mitologias
CREATE TRIGGER data_usages_mitologias_update
AFTER UPDATE ON mitologias
FOR EACH ROW
    UPDATE DATA_USAGES SET updates = updates + 1 WHERE usuario = CURRENT_USER();


-- Trigger para inserciones en mitologias
CREATE TRIGGER data_usages_mitologias_insert
AFTER INSERT ON mitologias
FOR EACH ROW

    UPDATE DATA_USAGES SET inserts = inserts + 1 WHERE usuario = CURRENT_USER();


-- Trigger para eliminaciones en mitologias
CREATE TRIGGER data_usages_mitologias_delete
AFTER DELETE ON mitologias
FOR EACH ROW

    UPDATE DATA_USAGES SET deletes = deletes + 1 WHERE usuario = CURRENT_USER();

-- Trigger para actualizaciones en tipos_creaturas
CREATE TRIGGER data_usages_tipos_creaturas_update
AFTER UPDATE ON tipos_creaturas
FOR EACH ROW
    UPDATE DATA_USAGES SET updates = updates + 1 WHERE usuario = CURRENT_USER();

-- Trigger para inserciones en tipos_creaturas
CREATE TRIGGER data_usages_tipos_creaturas_insert
AFTER INSERT ON tipos_creaturas
FOR EACH ROW
    UPDATE DATA_USAGES SET inserts = inserts + 1 WHERE usuario = CURRENT_USER();

-- Trigger para eliminaciones en tipos_creaturas
CREATE TRIGGER data_usages_tipos_creaturas_delete
AFTER DELETE ON tipos_creaturas
FOR EACH ROW
    UPDATE DATA_USAGES SET deletes = deletes + 1 WHERE usuario = CURRENT_USER();
...
-- Trigger para actualizaciones en clasificacion_criaturas
CREATE TRIGGER data_usages_clasificacion_criaturas_update
AFTER UPDATE ON clasificacion_criaturas
FOR EACH ROW
    UPDATE DATA_USAGES SET updates = updates + 1 WHERE usuario = CURRENT_USER();

-- Trigger para inserciones en clasificacion_criaturas
CREATE TRIGGER data_usages_clasificacion_criaturas_insert
AFTER INSERT ON clasificacion_criaturas
FOR EACH ROW
    UPDATE DATA_USAGES SET inserts = inserts + 1 WHERE usuario = CURRENT_USER();

-- Trigger para eliminaciones en clasificacion_criaturas
CREATE TRIGGER data_usages_clasificacion_criaturas_delete
AFTER DELETE ON clasificacion_criaturas
FOR EACH ROW
    UPDATE DATA_USAGES SET deletes = deletes + 1 WHERE usuario = CURRENT_USER();


-- Trigger para actualizaciones en nivel_riesgo
CREATE TRIGGER data_usages_nivel_riesgo_update
AFTER UPDATE ON nivel_riesgo
FOR EACH ROW
    UPDATE DATA_USAGES SET updates = updates + 1 WHERE usuario = CURRENT_USER();

-- Trigger para inserciones en nivel_riesgo
CREATE TRIGGER data_usages_nivel_riesgo_insert
AFTER INSERT ON nivel_riesgo
FOR EACH ROW
    UPDATE DATA_USAGES SET inserts = inserts + 1 WHERE usuario = CURRENT_USER();

-- Trigger para eliminaciones en nivel_riesgo
CREATE TRIGGER data_usages_nivel_riesgo_delete
AFTER DELETE ON nivel_riesgo
FOR EACH ROW
    UPDATE DATA_USAGES SET deletes = deletes + 1 WHERE usuario = CURRENT_USER();


-- Trigger para actualizaciones en caracteristicas
CREATE TRIGGER data_usages_caracteristicas_update
AFTER UPDATE ON caracteristicas
FOR EACH ROW
    UPDATE DATA_USAGES SET updates = updates + 1 WHERE usuario = CURRENT_USER();

-- Trigger para inserciones en caracteristicas
CREATE TRIGGER data_usages_caracteristicas_insert
AFTER INSERT ON caracteristicas
FOR EACH ROW
    UPDATE DATA_USAGES SET inserts = inserts + 1 WHERE usuario = CURRENT_USER();

-- Trigger para eliminaciones en caracteristicas
CREATE TRIGGER data_usages_caracteristicas_delete
AFTER DELETE ON caracteristicas
FOR EACH ROW
    UPDATE DATA_USAGES SET deletes = deletes + 1 WHERE usuario = CURRENT_USER();


-- Trigger para actualizaciones en criaturas
CREATE TRIGGER data_usages_criaturas_update
AFTER UPDATE ON criaturas
FOR EACH ROW
    UPDATE DATA_USAGES SET updates = updates + 1 WHERE usuario = CURRENT_USER();

-- Trigger para inserciones en criaturas
CREATE TRIGGER data_usages_criaturas_insert
AFTER INSERT ON criaturas
FOR EACH ROW
    UPDATE DATA_USAGES SET inserts = inserts + 1 WHERE usuario = CURRENT_USER();

-- Trigger para eliminaciones en criaturas
CREATE TRIGGER data_usages_criaturas_delete
AFTER DELETE ON criaturas
FOR EACH ROW
    UPDATE DATA_USAGES SET deletes = deletes + 1 WHERE usuario = CURRENT_USER();

-- Trigger para actualizaciones en evidencias
CREATE TRIGGER data_usages_evidencias_update
AFTER UPDATE ON evidencias
FOR EACH ROW
    UPDATE DATA_USAGES SET updates = updates + 1 WHERE usuario = CURRENT_USER();

-- Trigger para inserciones en evidencias
CREATE TRIGGER data_usages_evidencias_insert
AFTER INSERT ON evidencias
FOR EACH ROW
    UPDATE DATA_USAGES SET inserts = inserts + 1 WHERE usuario = CURRENT_USER();

-- Trigger para eliminaciones en evidencias
CREATE TRIGGER data_usages_evidencias_delete
AFTER DELETE ON evidencias
FOR EACH ROW
    UPDATE DATA_USAGES SET deletes = deletes + 1 WHERE usuario = CURRENT_USER();

-- Trigger para actualizaciones en testigos
CREATE TRIGGER data_usages_testigos_update
AFTER UPDATE ON testigos
FOR EACH ROW
    UPDATE DATA_USAGES SET updates = updates + 1 WHERE usuario = CURRENT_USER();

-- Trigger para inserciones en testigos
CREATE TRIGGER data_usages_testigos_insert
AFTER INSERT ON testigos
FOR EACH ROW
    UPDATE DATA_USAGES SET inserts = inserts + 1 WHERE usuario = CURRENT_USER();

-- Trigger para eliminaciones en testigos
CREATE TRIGGER data_usages_testigos_delete
AFTER DELETE ON testigos
FOR EACH ROW
    UPDATE DATA_USAGES SET deletes = deletes + 1 WHERE usuario = CURRENT_USER();

-- Trigger para actualizaciones en testimonios
CREATE TRIGGER data_usages_testimonios_update
AFTER UPDATE ON testimonios
FOR EACH ROW
    UPDATE DATA_USAGES SET updates = updates + 1 WHERE usuario = CURRENT_USER();

-- Trigger para inserciones en testimonios
CREATE TRIGGER data_usages_testimonios_insert
AFTER INSERT ON testimonios
FOR EACH ROW
    UPDATE DATA_USAGES SET inserts = inserts + 1 WHERE usuario = CURRENT_USER();

-- Trigger para eliminaciones en testimonios
CREATE TRIGGER data_usages_testimonios_delete
AFTER DELETE ON testimonios
FOR EACH ROW
    UPDATE DATA_USAGES SET deletes = deletes + 1 WHERE usuario = CURRENT_USER();

-- Trigger para actualizaciones en ubicaciones
CREATE TRIGGER data_usages_ubicaciones_update
AFTER UPDATE ON ubicaciones
FOR EACH ROW
    UPDATE DATA_USAGES SET updates = updates + 1 WHERE usuario = CURRENT_USER();

-- Trigger para inserciones en ubicaciones
CREATE TRIGGER data_usages_ubicaciones_insert
AFTER INSERT ON ubicaciones
FOR EACH ROW
    UPDATE DATA_USAGES SET inserts = inserts + 1 WHERE usuario = CURRENT_USER();

-- Trigger para eliminaciones en ubicaciones
CREATE TRIGGER data_usages_ubicaciones_delete
AFTER DELETE ON ubicaciones
FOR EACH ROW
    UPDATE DATA_USAGES SET deletes = deletes + 1 WHERE usuario = CURRENT_USER();

-- Trigger para actualizaciones en avistamientos
CREATE TRIGGER data_usages_avistamientos_update
AFTER UPDATE ON avistamientos
FOR EACH ROW
    UPDATE DATA_USAGES SET updates = updates + 1 WHERE usuario = CURRENT_USER();

-- Trigger para inserciones en avistamientos
CREATE TRIGGER data_usages_avistamientos_insert
AFTER INSERT ON avistamientos
FOR EACH ROW
    UPDATE DATA_USAGES SET inserts = inserts + 1 WHERE usuario = CURRENT_USER();

-- Trigger para eliminaciones en avistamientos
CREATE TRIGGER data_usages_avistamientos_delete
AFTER DELETE ON avistamientos
FOR EACH ROW
    UPDATE DATA_USAGES SET deletes = deletes + 1 WHERE usuario = CURRENT_USER();
...
/* test */
/*
INSERT INTO DATA_USAGES (usuario, inserts, updates, deletes)
VALUES ('nombre_usuario', 0, 0, 0);

-- Prueba de actualización en tipos_creaturas
UPDATE tipos_creaturas SET nombre_tipo = 'Nuevo Nombre' WHERE tipo_id = 1;

-- Prueba de inserción en tipos_creaturas
INSERT INTO tipos_creaturas (nombre_tipo, descripcion) VALUES ('Nuevo Tipo', 'Descripción del Nuevo Tipo');

-- Prueba de eliminación en tipos_creaturas
DELETE FROM tipos_creaturas WHERE tipo_id = 1;


-- Prueba de actualización en mitologias
UPDATE mitologias SET nombre_tipo = 'Nuevo Nombre' WHERE mitologia_id = 1;

-- Prueba de inserción en mitologias
INSERT INTO mitologias (nombre_tipo, descripcion) VALUES ('Nueva Mitología', 'Descripción de la Nueva Mitología');

-- Prueba de eliminación en mitologias
DELETE FROM mitologias WHERE mitologia_id = 1;


-- Prueba de actualización en clasificacion_criaturas
UPDATE clasificacion_criaturas SET descripcion = 'Nueva Descripción' WHERE nombre_clasificacion_id = 'ABC123';

-- Prueba de inserción en clasificacion_criaturas
INSERT INTO clasificacion_criaturas (nombre_clasificacion_id, descripcion, tipo_creatura_id, mitologia_id) VALUES ('XYZ456', 'Descripción de la Nueva Clasificación', 1, 1);

-- Prueba de eliminación en clasificacion_criaturas
DELETE FROM clasificacion_criaturas WHERE nombre_clasificacion_id = 'ABC123';
*/
